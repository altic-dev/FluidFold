...
///   ~/Library/Logs/Dusk/folds.log          one report block per fold, newest last
///   ~/Library/Logs/Dusk/folds/fold-N.csv   one row per on-screen frame (last 30 folds kept)
///
...
final class FoldRecorder {
    ...
    static let directory = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Dusk")
