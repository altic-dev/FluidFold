...
func dlog(_ message: @autoclosure () -> String) {
    let m = message()
    Logger(subsystem: "com.altic.FluidFold", category: "app").log("\(m, privacy: .public)")
    guard UserDefaults.standard.bool(forKey: "debugLog") else { return }
    let time = Date()
    logQueue.async {
        let url = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/FluidFold.log")
        let line = "\(time) \(m)\n"
